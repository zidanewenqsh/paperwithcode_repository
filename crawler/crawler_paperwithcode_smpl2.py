...
if __name__ == "__main__":
    resultpath = Path("./output/result2.txt")
    ...
    if 0:
        for i in range(1, 100):
            ...
            # 获取目标页面并处理
            result = spider.resp(target_url, False, treatbase, [hrefxpath, imgxpath, titlexpath], pattern)
            if len(result) == 0:
                break
        ...
        resultsdict = spider.resp(None, True, treatbase, [hrefxpath, imgxpath, titlexpath], pattern)
        # print(type(results))
    ...
    for j, title in enumerate(resultsdict.keys()):
        ...
        ret = spider.resp(url, False, treatsubdata, [abstractxpath1, pdfxpath1, codexpath1], "")
        ...
        ret = spider.resp(url, False, treatsubdata_dataset, [hrefxpath1, imgxpath1, titlexpath1], "")
        if len(ret) > 0:
