...
class CodebookBackgroundSubtractor:
    def __init__(self, n_clusters=5, learning_rate=0.1):
    ...
    def detect_foreground(self, frame):
        ...
        distances = np.abs(pixels - self.kmeans.predict(pixels).reshape(-1, 1))
        foreground_mask = np.sum(distances > 10, axis=1) > 0  # 阈值判断是否为前景
...
cap = cv2.VideoCapture(videopath)  # 或者直接使用摄像头 cap = cv2.VideoCapture(0)
bg_subtractor = CodebookBackgroundSubtractor(n_clusters=5)
